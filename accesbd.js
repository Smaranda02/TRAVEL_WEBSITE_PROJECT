...
//js are si el garbage collector

class AccesBD{
    static #instanta=null;  // # inseamna private, fara # inseamna public
    static #initializat=false;

    constructor() {
        if(AccesBD.#instanta){    //vrem un singleton
            throw new Error("Deja a fost instantiat");
        }
        else if(!AccesBD.#initializat){  //atributele statice le accesem cu numele clasei
            throw new Error("Trebuie apelat doar din getInstanta; fara sa fi aruncat vreo eroare");
        }
    }

    initLocal(){   //se creeaza clientul
        this.client= new Client({database:"ProiectWBBD",
            user:"zmara",
            password:"admin2002",
            host:"localhost", 
            port:5432});
     ...
     * @returns {AccesBD}
     */
    static getInstanta({init="local"}={}){  //{init="local" e un obiect default cu param=val
    ...
    insert({tabel="",campuri={}} = {}, callback){
        ...
        let comanda=`insert into ${tabel}(${Object.keys(campuri).join(",")}) values ( ${Object.values(campuri).map((x) => `'${x}'`).join(",")})`;   //fct map ia fiecare camp din vector (x) si il inlocuieste cu '{x}';  => definesteun lamda function
        console.log(comanda);
    ...
    update({tabel="",campuri={}, conditiiAnd=[]} = {}, callback, parametriQuery){
        let campuriActualizate=[];
        for(let prop in campuri)  //dif intre iterare cu in vs cu of este ca : cu of iteram prin vector ( prin valoare), iar in itereaza prin vect de proprietati
